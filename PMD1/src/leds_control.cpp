...
void init_secuence_control(SECUENCE_CONTROL* sequence_control, LED* led_sequence, uint8_t led_sequence_size, uint16_t* delay_values, uint8_t delay_values_size) {
    sequence_control->direction = true;
    sequence_control->delay = delay_values[0];
...
void update_sequence(SECUENCE_CONTROL* sequence_control){
    ...
    if (!led_state and nb_delay(sequence_control->delay)) {
        digitalWrite(sequence_control->led_sequence[current_led].pin, HIGH);
        led_state = true;
    } else if(led_state and nb_delay(sequence_control->delay)) {
